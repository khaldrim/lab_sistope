 ...
 * Description: 
 */
BMPINFOHEADER *ReadBMPWinInfoHeader(FILE *fp, BMPINFOWINHEADER *header)
{
    ...
    header->colorPlanes  = planes;
    header->bitsPerPixel = bitcount;
    header->compression  = compression;
    header->imgSize      = size_image;
    header->xPixPerMeter = x_pix_per_meter;
    header->yPixPerMeter = y_pix_per_meter;
    header->colorPalette = clr_palette;
    header->colorUsed    = clr_used;
...
FILE* readImageHeader(int imgCount, FILE *fp,BMPFILEHEADER *bmpfh, BMPINFOOSHEADER *bmpOSIH, BMPINFOWINHEADER *bmpWinIH)
{
    ...
    {
        bmpWinIH = ReadBMPInfoHeader(fp, bmpWinIH);
    }
    else
    {
